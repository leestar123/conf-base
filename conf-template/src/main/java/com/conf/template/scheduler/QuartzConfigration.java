...
public class QuartzConfigration {
    ...
    public MethodInvokingJobDetailFactoryBean detailFactoryBean(ScheduleTask task) {
        ...
        jobDetail.setGroup("scheduler_conf");// 设置任务的分组，这些属性都可以存储在数据库中，在多任务的时候使用  

        /* 
         * 这两行代码表示执行task对象中的schedule方法。定时执行的逻辑都在schedule。
         */  
    ...
    public CronTriggerFactoryBean cronJobTrigger(MethodInvokingJobDetailFactoryBean jobDetail) {  
        ...
        tigger.setCronExpression("0/20 * * * * ?");// 表示每隔20秒钟执行一次
        //tigger.set
